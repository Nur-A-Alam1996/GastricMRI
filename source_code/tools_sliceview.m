%%
% tools_sliceview: view mri/fmri dataset as slices
%
% Usage
%   [uimg,oimg]=tools_sliceview(ulay [,olay,'keyword','value'...]);
%   [uimg,oimg]=tools_sliceview(ulay,olay,'ui',1);
%
% Inputs
%   ulay: 3D or 4D array
%   olay: 3D or 4D array
...
function tools_sliceview(ulay,varargin)

if nargin<1
    eval('help tools_sliceview');
    return
end

if ndims(ulay)<2
    error('tools_sliceview(): ulay dimensions must be larger than 1');
...
flag_ui=0;         % display user interface
...
for i=kwstart:2:size(varargin,2)
    ...
    Value   = varargin{i+1};
    if ~ischar(Keyword) 
        printf('tools_sliceview(): keywords must be strings')
        return
    end
...
%         ocgap=Value(1);
        ...
        fprintf(['tools_sliceview(): ' Keyword ' is an unknown keyword']);
    end
end
